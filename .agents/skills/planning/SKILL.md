---
name: planning
description: Cut a dev-ready version into an ordered set of implementation phases and write the version plan (docs/handoff/<version>/<version>-plan.md) that each phase's Spec Loop runs over. Use when starting a new version, breaking a version's FRs into phases, or writing/updating a <version>-plan.md - e.g. "plan v0.4", "break v0.4 into phases", "start the v0.4 plan".
---

# dev-ready Version Planner

Produce `docs/handoff/<version>/<version>-plan.md`: the authoritative breakdown
of a version into ordered phases, written wearing the Tech Lead hat. Each phase's
Spec Loop starts by reading it, so it must name concrete FRs, ADRs,
`src/dev_ready/...` paths, and acceptance criteria — not intentions.
...
Since ADR-021 (internal process v3), each phase is *executed* through the Spec
Loop and nothing else: `grill-with-docs` → `to-spec` (a committed spec in
`docs/specs/<version>/`) → `to-tickets` → `implement` (review included). No gate
documents are generated. The version plan cut here is the map those loops run
over; its phase sections should name the FR each spec will cover.
...
- One phase = one coherent, independently reviewable slice that leaves `main`
  releasable-in-principle (tests green), sized to a focused evening. Moofon has
...
- Exactly one phase per version ships the release, and it is the LAST phase. Its
  section must say so and name the version bump (e.g. "bump 0.2.2 → 0.3.0") —
  that is what tells the `release` skill which phases the release covers.
- State cross-phase couplings explicitly as "do NOT do X here — phase N owns it",
  so each phase's spec can carry the exclusion.
...
- A title line `## Phase <N> — <summary> (FR-…)` and, if done, a `— COMPLETED
  <date>` marker naming the commits that closed it.
- The concrete change surface: catalog/manifest entries, `src/dev_ready/...`
  files touched, overlay/verify behavior, with VERIFY-AT-IMPLEMENTATION markers
  for anything that must be checked against a real registry/API at implementation
...
unit tests, Conventional Commits, no state-changing git without Moofon's explicit
per-action permission), any "no new runtime dependencies" rule, module-boundary
...
- Header: `Status: Accepted (<date>). Written wearing the Tech Lead hat
  (ADR-021). Source of truth for the version's phase cut.` plus the scope line
  (FR range + ADRs) and the ship-as/baseline line.
- Tell Moofon the next step: for each phase in order, run the Spec Loop —
  `grill-with-docs` → `to-spec` (spec accepted before moving on) → `to-tickets`
  → `implement` — starting with Phase 1.
