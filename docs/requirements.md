...
- FR-12 (v0.3). Codebase-memory MCP item in the `mcp` component (reference mode, no redistribution).
- FR-13 (v0.3). react-doctor wrapper skill item in the `skills` component (original text; runs `npx react-doctor@<pinned-major>`; no vendored code).
...
3. v0.3: reference integrations + selection — generation stamp (FR-11), codebase-memory MCP (FR-12), react-doctor wrapper skill (FR-13), item-level selection (FR-14). See [version-plan.md](version-plan.md).
