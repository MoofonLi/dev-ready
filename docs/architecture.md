...
- Runtime dependencies are kept minimal (current: questionary, copier, rich — see ADR-005). Every new dependency requires a note here. **rich** (added v0.13, ADR-003's 2026-08-23 amendment) renders the static screens — the pre-generation confirmation, the Engineering Flow comparison, and the generation report — supplying the TTY detection, `NO_COLOR` handling, and plain-text degradation FR-44 declined to author by hand. `questionary` keeps every interactive prompt: it owns keyboard handling, cancellation, and disabled rows. `render_report` stays pure and returns a string; rich renders into it, and the TTY/`NO_COLOR` decision stays at the `cli` boundary. Dev-only: `prompt-toolkit`, pinned `>=3.0.29,<4` so tests can drive `_questionary_asker` headlessly over `create_pipe_input()`/`DummyOutput()`; questionary already pulls it in, but at `>=2.0,<4.0` — too loose for an API the tests call directly.
