...
| [ADR-031](decisions/adr-031-occupied-target-directory.md) | `init` may generate into an occupied directory when nothing collides at the top level (v0.13) | Accepted (2026-08-23) |
...
| `prompts` | Collect Generation Intent via the terminal, and parse selection flags | perform I/O other than terminal |
| `intent` | Hold Generation Intent: project name, destination, catalog selection, Agent Targets | perform I/O, parse CLI flag strings, or import `prompts` |
...
| `overlay` | Apply dev-ready files onto the fetched base; templating of names/values, Mount Point injection, and catalog-declared Flow Convention and Setup Contribution (overlay-interpolated, never destination writes) | fetch anything from the network |
...
| `presentation` | Render the shared frameless Static Screen idiom and detect stream capabilities as primitive colour/width values | touch the filesystem, perform network I/O, or decide terminal policy |
...
- Direction: `cli` -> `prompts`/`intent`/`manifest`/`fetch`/`overlay`/`report`/`verify`. Lower modules never import `cli`. `prompts` may depend on `intent`; `intent` does not import `prompts`.
...
- Runtime dependencies are kept minimal (current: questionary, copier, rich — see ADR-005). Every new dependency requires a note here. **rich** (added v0.13, ADR-003's 2026-08-23 amendment) renders the static screens — the pre-generation confirmation, the Engineering Flow comparison, and the generation report — supplying the TTY detection, `NO_COLOR` handling, and plain-text degradation FR-44 declined to author by hand. Its resolved runtime tree is four packages: `rich`, `markdown-it-py`, `mdurl`, and `pygments`. `questionary` keeps every interactive prompt: it owns keyboard handling, cancellation, and disabled rows. `render_report` stays pure and returns a string; rich renders into it. The `presentation` helper detects stream capabilities, while `cli` owns terminal policy by deciding when to call that helper and distributing the resulting primitive values. Dev-only: `prompt-toolkit`, pinned `>=3.0.29,<4` so tests can drive `_questionary_asker` headlessly over `create_pipe_input()`/`DummyOutput()`; questionary already pulls it in, but at `>=2.0,<4.0` — too loose for an API the tests call directly.
