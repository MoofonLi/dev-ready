...
**Recorded Capture**:
The terminal image a README shows, produced by replaying a committed tape
against the real CLI rather than being drawn or screenshotted by hand. The tape
is the artifact under version control; the image is its output and is
regenerated by re-running it. A Recorded Capture is the one surface dev-ready
speaks on that ADR-023's guard cannot read, so no claim may rest on it alone.
_Avoid_: screenshot, demo gif, terminal mockup
